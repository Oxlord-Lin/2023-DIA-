...
summary(model)
library(broom)
res_mod <- as.data.frame(tidy(model))
write.csv(res_mod, "res_mod1.csv")
# Call:
...
t.fat = table(data24.fat$treat,data24.fat$y1)
control.fat.rate = t.fat[1,2]/sum(t.fat[1,])
case.fat.rate = t.fat[2,2]/sum(t.fat[2,])
fat.rate = c(control.fat.rate,case.fat.rate)

t.thin = table(data24.thin$treat,data24.thin$y1)
control.thin.rate = t.thin[1,2]/sum(t.thin[1,])
case.thin.rate = t.thin[2,2]/sum(t.thin[2,])
thin.rate = c(control.thin.rate,case.thin.rate)

t.all = table(data24$treat,data24$y1)
control.rate = t.all[1,2]/sum(t.all[1,])
case.rate = t.all[2,2]/sum(t.all[2,])
all.rate = c(control.rate,case.rate)
